...
let DefaultImageEventPerformerCache:performerCache = {runningTrace: 0, maxTrace: 0,pendingTrace:{total:0}}
let DefaultCallUipathAPIEventPerformerCache:performerCache =  {runningTrace:0,maxTrace:1,pendingTrace:{total:0}}
let IsInit = false;
...
const addListenerToPendingQueue= (snapshot_queueKey: string)=>{
    const triggerPath = ImageEventPerformerName+"/pendingTrace/traceIDs/data/"+snapshot_queueKey
    const timeout = setTimeout(() => {
        console.log(`listener child_removed on ${triggerPath} timeout exceed at 10000ms`)
    ...
    PerformerCaches.child(triggerPath).once('child_removed',(pendingDataSnapshot:DataSnapshot)=>{
        ...
        LineTransactionQueue.child(snapshot_queueKey).transaction((pendingTransactionSnapshot:LineImageEventTransactionInfo)=>{
            ...
            if(committed){
                PerformerCaches.child(ImageEventPerformerName+"/pendingTrace/total").transaction((totalTransactionUpdate:number|null)=>{
                    if(!IsInit)return;
                    const defaultCache = DefaultImageEventPerformerCache.pendingTrace.total;
                    if(totalTransactionUpdate==undefined)return defaultCache;
...
LineTransactionQueue.orderByChild("state").equalTo(transactionState.new).on('child_added',async (snapshot_queue:DataSnapshot) => {
    ...
    PerformerCaches.child(ImageEventPerformerName).transaction((currentCache:performerCache) => {
        if(!IsInit)return;
        if (!currentCache) {
        ...
        console.log(`Error LineTransactionQueue.orderByChild("state").equalTo(transactionState.new).on`,err.message||err);
    }
...
LineTransactionQueue.orderByChild("state").equalTo(transactionState.finalize).on('child_added',async (snapshot_queue:DataSnapshot) => {
    if(!IsInit)return;
    try{
        if(!snapshot_queue.exists())return;
        if(snapshot_queue.key == null)return;
        ...
        PerformerCaches.child(ImageEventPerformerName).transaction((currentCache:performerCache) => {
            ...
            if(currentCache.pendingTrace.traceIDs && currentCache.pendingTrace.traceIDs.data && currentCache.pendingTrace.traceIDs.last && currentCache.pendingTrace.traceIDs.first){
                ...
                if(!nextNode){
                    currentCache.pendingTrace=DefaultImageEventPerformerCache.pendingTrace;
                }else{
                    //delete first
        ...
        console.log(`Error LineTransactionQueue.orderByChild("state").equalTo(transactionState.finalize).on`);
    }
    
});
//for maintain storage , delete went reach a time.
...
LineTransactionQueue.orderByChild("state").equalTo(transactionState.pending).on('child_added',async (snapshot_queue:DataSnapshot) => {
    ...
    const snapshot_queueKey: string = snapshot_queue.key;
    const snapshot_queueData: LineImageEventTransactionInfo = snapshot_queue.val();
    console.log("Pending image: " + snapshot_queueKey);
    ...
    PerformerCaches.child(ImageEventPerformerName).transaction((currentCache:performerCache) => {
        ...
        if (error) {
            ...
            addListenerToPendingQueue(snapshot_queueKey);
        }
    });
    }catch(err:any){
        console.log(`Error LineTransactionQueue.orderByChild("state").equalTo(transactionState.pending).on`,err.message||err);
    }
...
()=> ServerInstanceDatabase.once('value',async(currentCacheSnapshot) => {
    const currentCache= currentCacheSnapshot.val()
    ...
    if(currentCache[strImageTransactions]){
        ...
        Object.keys(imageTransaction).map(childKey => {
    ...
    if(currentCache[strUiPathAPITransactions]){
        ...
        Object.keys(uipathTransaction).map(childKey => {
        ...
        currentCache[strImageTransactions] = uipathTransaction;
        
    }
    const updateCache = {[ImageEventPerformerName]:DefaultImageEventPerformerCache,[CallUipathAPIEventPerformerName]:DefaultCallUipathAPIEventPerformerCache}
